...
- YOLO 推理：支持 `cuda` 和 `vulkan` 两种后端。
- 模型格式切换：`cuda` 使用 `.pt` 权重，`vulkan` 使用 Ultralytics 导出的 NCNN 模型目录。
...
- AMD / Intel / 其他支持 Vulkan 的设备可尝试 `vulkan` 后端
...
如果 `cuda` 后端无法使用 GPU，先确认当前环境中的 PyTorch 是否安装了匹配显卡驱动的 CUDA 版本。`ultralytics` 会依赖 PyTorch，但具体 CUDA wheel 可能需要按本机环境单独安装。
...
3. 选择推理后端：NVIDIA 通常用 `cuda`，其他 GPU 可尝试 `vulkan`。
4. 点击“选择模型文件”确认模型路径。
5. 点击“打开视频显示”启动画面预览。
6. 点击“开启 YOLO”启动检测。
...
| 配置项 | 作用 |
| --- | --- |
| `inference_backend` | 推理后端，支持 `cuda` / `vulkan` |
