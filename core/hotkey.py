...
VK_CODE_MAP = {
    ...
    "VK_BACK": 0x08, "VK_TAB": 0x09, "VK_CLEAR": 0x0C,
    "VK_RETURN": 0x0D, "VK_SHIFT": 0x10, "VK_CONTROL": 0x11,
    "VK_MENU": 0x12, "VK_PAUSE": 0x13, "VK_CAPITAL": 0x14,
    "VK_KANA": 0x15, "VK_HANGUL": 0x15, "VK_IME_ON": 0x16,
    "VK_JUNJA": 0x17, "VK_FINAL": 0x18, "VK_HANJA": 0x19,
    "VK_KANJI": 0x19, "VK_IME_OFF": 0x1A, "VK_ESCAPE": 0x1B,
...
_CODE_TO_NAME = {v: k for k, v in VK_CODE_MAP.items()}
...
class _KeyCaptureWindow:
    def __init__(self, root: tk.Tk):
        self._root = root
        self._root.title("Key Capture")
        self._label = tk.Label(root, text="Press any key...", font=("Helvetica", 16))
    ...
    def _on_key(self, key):
        ...
        if isinstance(key, kb.KeyCode):
            self._result = hex(key.vk) if key.vk is not None else "UNKNOWN"
        else:
            self._result = hex(key.value.vk)
        self._close()

    def _on_click(self, _x, _y, button, _pressed):
        vk = _MOUSE_VK.get(button)
        self._result = hex(vk) if vk else "UNKNOWN"
        self._close()
...
def capture_hotkey(fallback: str = "UNKNOWN") -> str:
    result_box: queue.Queue = queue.Queue()

    def _worker():
        result = _KeyCaptureWindow(tk.Tk()).run()
    ...
    captured = "UNKNOWN" if result_box.empty() else result_box.get()
    return fallback if captured is None or captured == "UNKNOWN" else captured


def vk_code_to_name(hex_str: str) -> str:
    return _CODE_TO_NAME.get(int(hex_str, 16), "UNKNOWN")


def vk_name_to_code(name: str) -> int | str:
    return VK_CODE_MAP.get(name, "UNKNOWN")
